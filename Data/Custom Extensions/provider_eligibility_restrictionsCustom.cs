    ...
    public partial class provider_eligibility_restrictions : DataObject
    {
        ...
        public static List<provider_eligibility_restrictions> GetAllDataAsList(bool enabledOnly = true)
        {
            ...
            foreach (DataRow aRow in dt.Rows)
            {
                ...
                {
                    var initialize = per.LinkedFilters.Count;
                    toReturn.Add(per);
                }
            }
            ...
            if (newProvider == string.Empty) // no changes to newProvider made
            {
                foreach (provider_eligibility_restrictions restriction in restrictionsList)
                {
                    ...
                    if (checkValid)
                    {
                        ...
                        foreach (insurance_company_groups_filters filter in restriction.LinkedFilters)
                        {
                            // Starts with %
                            //   -- Closing % - CONTAINS
                            //   -- No closing % - ENDSWITH
        ...
        private List<insurance_company_groups_filters> GetLinkedCompanyFilters()
        {
            ...
            claim c = new claim();
            DataTable dt = c.Search("SELECT * FROM insurance_company_groups_filters WHERE icg_id IN(" + string.Join(",", Groups.Select(x => x.id)));
        ...
        public List<insurance_company_group> Groups
        {
            get
            {
                List<insurance_company_group> icgs = new List<insurance_company_group>();

                DataTable dt = Search(string.Format(
                    "SELECT insurance_group_id FROM provider_eligibility_restriction_groups where provider_eligibility_restriction_id = {0}", id));

                insurance_company_group toAdd;

                foreach (DataRow anICG in dt.Rows)
                {
                    toAdd = new insurance_company_group(Convert.ToInt32(anICG[0]));
                    icgs.Add(toAdd);
                }

                icgs = icgs.OrderBy(x => x.name).ToList();
                return icgs;
            }
        }
        ...
        /// <summary>
        /// Sets the groups for the current restriction to the given list (clears them all out then sets them).
        /// </summary>
        /// <param name="_includedGroups"></param>
        internal static void SetGroups(int restriction_id, List<insurance_company_group> _includedGroups)
        {
            // clear out existing groups
            new provider_eligibility_restrictions().ExecuteNonQuery(
                string.Format("DELETE FROM provider_eligibility_restriction_groups WHERE provider_eligibility_restriction_id = {0}", restriction_id));
            // add new groups
            _includedGroups.ForEach(g => { AddGroup(restriction_id, g.id, false); });
        }
        ...
        public List<insurance_company_groups_filters> LinkedFilters
        {
            get
            {
                List<insurance_company_groups_filters> allFilters = new List<insurance_company_groups_filters>();

                Groups.ForEach(g =>
                {
                    allFilters.AddRange(g.Filters);
                });
