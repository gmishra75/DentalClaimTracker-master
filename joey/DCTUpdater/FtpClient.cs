 ...
 * Adapted for use by Aaron Johnson 08/08/06
 */
...
namespace DCTUpdater
{
	public class FtpClient
	{
		...
		public FtpClient()
		{
			this.server = DCTUpdater.Properties.Settings.Default.FTPServer;
			this.username = DCTUpdater.Properties.Settings.Default.FTPUserName;
			this.password = DCTUpdater.Properties.Settings.Default.FTPPassword;
		}
		/// <summary>
		/// 
		/// </summary>
		...
		public FtpClient(string server, string username, string password, bool temp)
		{
			this.server = server;
		...
		public void Download(string locFileName)
		{
			this.Download(locFileName,false);
		}

		/// <summary>
		/// Download working file to a local file name which can include
		...
		public void Download(string locFileName,Boolean resume)
		{
			if ( !this.loggedin ) this.Login();

			this.BinaryMode = true;

			Debug.WriteLine("Downloading file " + WorkingFile + " from " + server + "/" + remotePath, "FtpClient" );

			if (locFileName.Equals(""))
			{
				throw new Exception("Application error. Invalid path specified.");
			}
			...
			this.sendCommand("RETR " + WorkingFile);
		...
		/// <param name="fileName"></param>
		/// <param name="resume"></param>
		public void Upload(bool resume)
		{
			Debug.WriteLine("Uploading File 1. " + WorkingFile, "FtpClient");
			// if ( !this.loggedin ) this.Login();
			Socket cSocket = null ;
			long offset = 0;
			...
			if ( resume )
			{
				try
				{
					...
					offset = GetFileSize( Path.GetFileName(WorkingFile) );
				}
				catch
				{
					// file not exist
			...
			FileStream input = new FileStream(WorkingFile,FileMode.Open);

			if ( resume && input.Length < offset )
			{
				// different file size
				Debug.WriteLine("Overwriting " + WorkingFile, "FtpClient");
				offset = 0;
			}
			else if ( resume && input.Length == offset )
			{
				// file done
				input.Close();
				Debug.WriteLine("Skipping completed " + WorkingFile + " - turn resume off to not detect.", "FtpClient");
				return;
			}
			...
			this.sendCommand( "STOR " + Path.GetFileName(WorkingFile) );
			...
			Debug.WriteLine( "Uploading file " + WorkingFile + " to " + remotePath, "FtpClient" );
			...
			Debug.WriteLine("Upload of " + WorkingFile + " complete.", "FtpClient");
		...
		public void DeleteFile()
		{
			...
			this.sendCommand( "DELE " + WorkingFile );

			if ( this.resultCode != 250 ) throw new FtpException(this.result.Substring(4));

			Debug.WriteLine( "Deleted file " + WorkingFile, "FtpClient" );
		}
...
*/
		...
		private delegate void DownloadCallback(String locFileName);
		public System.IAsyncResult BeginDownload(string locFileName, System.AsyncCallback callback, object StateObject)
		{
			DownloadCallback ftpCallback = new DownloadCallback(this.Download);
			return ftpCallback.BeginInvoke(locFileName, callback, StateObject);
		}
		
		
		
		private delegate void DownloadFileNameResumeCallback(String locFileName,Boolean resume);
		public System.IAsyncResult BeginDownload(string locFileName, Boolean resume, System.AsyncCallback callback, object StateObject)
		{
			DownloadFileNameResumeCallback ftpCallback = new DownloadFileNameResumeCallback(this.Download);
			return ftpCallback.BeginInvoke(locFileName, resume, callback, StateObject);
		}




		private delegate void UploadCallback();
		public System.IAsyncResult BeginUpload(System.AsyncCallback callback )
		{
			UploadCallback ftpCallback = new UploadCallback(this.Upload);
			return ftpCallback.BeginInvoke(callback, null);
		}
		public System.IAsyncResult BeginUpload(System.AsyncCallback callback, object StateObject )
		{
			UploadCallback ftpCallback = new UploadCallback(this.Upload);
			return ftpCallback.BeginInvoke(callback, StateObject);
		}

		private delegate void UploadFileNameResumeCallback(Boolean resume);
		public System.IAsyncResult BeginUpload(Boolean resume, System.AsyncCallback callback )
		{
			UploadFileNameResumeCallback ftpCallback = new UploadFileNameResumeCallback(this.Upload);
			return ftpCallback.BeginInvoke(resume, callback, null);
		}
		...
		private delegate void DeleteFileCallback();
