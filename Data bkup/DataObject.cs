...
namespace C_DentalClaimTracker
{
    ...
    public abstract class DataObject
    {
        ...
        private OdbcCommand _dbcmdData;
        private OdbcDataAdapter _dbadpData;
        ...
        public class PrimaryFieldsCollection : CollectionBase
        {
            ...
            public void Remove(int index)
            {
                ...
                {
                    // Invalid index
                    LoggingHelper.Log("Cannot remove index of " + index + " in DataObject.Remove(int index)", LogSeverity.Critical);
            ...
            public PrimaryField this[int index]
            {
                ...
                set
                {
                    if (index > Count - 1 || index < 0)
                    {
                        LoggingHelper.Log("Invalid Primary Key Index, DataObject.PrimaryField", LogSeverity.Critical);
                        // Invalid index						
        ...
        public void DefaultInitialization(string Table)
        {
            ...
            if (_connectionHandler == null)
            {
                ...
                // _connectionHandler.AddConnection(_connectionAlias, true); // Persistent Connection should be true
            }
            ...
            _dbcmdData = new OdbcCommand();
            _dbadpData = new OdbcDataAdapter(_dbcmdData);
        ...
        private void BuildSql()
        {
            ...
            for (int i = 0; i < _dataTable.Columns.Count; i++)
            {
                ...
                {
                    LoggingHelper.Log("Programmer error: unsupported data type " + _dataTable.Columns[i].DataType.ToString() + " in table.\n\n" + DataCommand.CommandText, LogSeverity.Critical);
                    // Unsupported Type
        ...
        {
            OdbcDataAdapter dbadpSearch = new OdbcDataAdapter(DataCommand);
            ...
            {
                LoggingHelper.Log("Programmer error. Error in search function. Invalid SQL.\n\n" + _searchSQL, LogSeverity.Critical);
                throw err;
            }
            finally
            {
        ...
        /// </summary>
        private OdbcCommand DataCommand
        {
            get 
            { return _dbcmdData; }
        ...
        public DataRow Row
        {
            get
            {
                if (_dataRow == null)
                {
                    LoggingHelper.Log("No record error, DataObject.Row", LogSeverity.Critical);
        ...
        public void Load(int ID)
        {
            ...
            try
            {
                ...
                if (x == 0)
                {
                    LoggingHelper.Log("Tried to load primary key '0' in DataObject.Load.", LogSeverity.Critical);
        ...
        public void Load(DataRow dataRow)
        {
            ...
            {
                LoggingHelper.Log("Load from a prepopulated data row failed.", LogSeverity.Critical);
        ...
        private ReservedKey GetNextKey()
        {
            ...
            {
                LoggingHelper.Log("Error in DataObject.GetNextKey\n" + err.Message, LogSeverity.Critical);
                throw err;
            }
            finally
            {
            ...
            try
            {
                ...
                if (_dataRow.RowState == DataRowState.Detached)
                {
                    ...
                    for (int i = 0; i < _dataTable.Columns.Count; i++)
                    {
                       ...
                       || (_dataTable.Columns[i].DataType == System.Type.GetType("System.Int64"))
                        ...
                        {
                            LoggingHelper.Log("Programmer error in DataObject.Save: unsupported data type: " + _dataTable.Columns[i].DataType.ToString(), LogSeverity.Critical);
                ...
                else
                {
                    ...
                    for (int i = 0; i < PrimaryKeys.Count; i++)
                    {
                        ...
                        {
                            LoggingHelper.Log("Programmer error in DataObject.Save: unsupport data type: " + _dataTable.Columns[i].DataType.ToString(), LogSeverity.Critical);
                            DataObjectException InvalidDataType = new DataObjectException("Unsupported data type. [" + _dataTable.Columns[i].DataType.ToString() + "]");
                            throw InvalidDataType;
                        }
                ...
                while (true) // Loop until Break for success
                {
                    ...
                    catch (OdbcException err)
                    {
                        if (err.ErrorCode == -2147217873) // Primary Key violation
                        {
                            ...
                            savekey++;
                            DataCommand.CommandText = DataCommand.CommandText.Replace("VALUES(" + Row[PrimaryKeys[0].Name].ToString() + ",", "VALUES (" + savekey.Value.ToString() + ",");
                        }
                        else
                        {
                            LoggingHelper.Log(err);
        ...
        public void Zap()
        {
            ...
            {
                DataCommand.Connection = ConnHandler.RequestConnection(Alias, this);
                DataCommand.CommandText = "DELETE FROM " + TableName;
                DataCommand.ExecuteNonQuery();
            }
            catch (Exception err)
            {
        ...
        public virtual void Refresh()
        {
            try
            {
                OdbcConnection dcconn = DataCommand.Connection;
            ...
            {
                LoggingHelper.Log("Error in DataObject.Refresh: " + err.StackTrace, LogSeverity.Error, err);
                throw err;
            }
            finally
            {
