...
namespace C_DentalClaimTracker
{
    partial class claim_status_history
    {
        ...
        public string CreateChangeText()
        {
            ...
            {
                if (!old_revisit_date.HasValue)
                {
                    if (new_revisit_date.HasValue)
                    {
                        toReturn = "RV Date (none) -> " + new_revisit_date.Value.ToShortDateString();
                    }
                }
                else if (new_revisit_date.HasValue)
                {
                    if (old_revisit_date.Value != new_revisit_date.Value)
                    {
                        toReturn = "RV Date " + old_revisit_date.Value.ToShortDateString() + " -> " +
                            new_revisit_date.Value.ToShortDateString();
                    }
                }
                ...
                if (old_status_id != new_status_id)
                {
                    ...
                    if (old_status_id <= 0)
                        toReturn += "Status: (none) -> " + LinkedNewStatus.name;
                    else
                        toReturn += "Status: " + LinkedOldStatus.name + " -> " + LinkedNewStatus.name;
                }
