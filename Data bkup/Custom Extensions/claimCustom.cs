...
namespace C_DentalClaimTracker
{
    partial class claim
    {
        ...
        /// <summary>
        /// A custom rule that checks to see if there is a provider_eligibility restriction that assigns the provider to DHKO
        /// </summary>
        public bool CheckForAddressOverride
        {
            get
            {
                ...
                {
                    claim c = provider_eligibility_restrictions.FindEligibilityRestrictions(this);
                    if (c != null)
                        if (c.doctor_provider_id == "DHKO")
                            return true;
                }
        ...
        public List<call> GetPastCalls(bool requireAnswer)
        {
            ...
            if (requireAnswer)
                sql += " AND (Select count(*) FROM choices where call_id = c.id)";
        ...
        {
            ExecuteNonQuery("UPDATE claims SET import_update_flag = " + updated);
        }

        /// <summary>
        /// Closes all claims whose import update flag is set to false
        /// </summary>
        internal void CloseClaimsWithoutUpdate()
        {
            ExecuteNonQuery("UPDATE claims SET open = FALSE WHERE import_update_flag = FALSE");
        }
        ...
        /// <summary>
        /// Get/Set for the decimal amount_of_claim. Throws DataObjectException if the data is null.
        /// </summary>
        ...
        /// <summary>
        /// Get/Set for the int ClaimType. CUSTOM: Uses enum. Returns Primary on invalid data.
        /// </summary>
        ...
        internal bool SetResendStatus()
        {
            ...
            if ((revisitDate >= 0) || (resendStatus >= 0))
            {
                ...
                SetToNewStatus(new claim_status(newStatus), newRevisit);
        ...
        internal void CreateStatusHistory(int newStatus, DateTime? newRevisit, bool as_is_flag)
        {
        ...
        internal void SetToNewStatus(claim_status newStatus, DateTime? newDate)
        {
            string noteText = "";
            ...
            if (newStatus != null)
            {
                if (newStatus.id != status_id)
                {
                    ...
                    CreateStatusHistory(newStatus.id, actualDateValue, false);
