    ...
    public partial class user_action_log
    {
        ...
        internal void DeleteLastCall(int claimID, int userID)
        {
            ExecuteNonQuery("DELETE FROM user_action_log WHERE action_id = " + (int)C_DentalClaimTracker.ActiveUser.ActionTypes.StartCall +
                " AND user_id = " + userID + " AND claim_id = " + claimID + " ORDER BY order_id desc LIMIT 1");

            ExecuteNonQuery("DELETE FROM user_action_log WHERE action_id = " + (int)C_DentalClaimTracker.ActiveUser.ActionTypes.EndCall +
                " AND user_id = " + userID + " AND claim_id = " + claimID + " ORDER BY order_id desc LIMIT 1");
        }
        ...
        internal static user_action_log FindMostRecent(int claimID, int userID)
        {
            ...
            DataTable matches = toReturn.Search("SELECT * FROM user_action_log WHERE action_id = " + (int)C_DentalClaimTracker.ActiveUser.ActionTypes.ViewClaim +
                " AND user_id = " + userID + " AND claim_id = " + claimID + " ORDER BY order_id desc LIMIT 1");
